// tracker.js (GitHub Pages / static)
...
(() => {
  ...
  function render() {
    ...
    if (todayLabel) todayLabel.textContent = `Date: ${today}`;
  ...
  function drawWeekChart(habits, today) {
    ...
    const gap = 10;
    const barW = Math.max(10, Math.floor(w / days.length) - gap);
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    if (!habitForm || !habitInput || !clearTodayBtn || !resetAllBtn) {
      // If tracker elements aren't on this page, do nothing
      return;
    }
    ...
    resetAllBtn.addEventListener("click", () => {
      if (confirm("Reset all habits? This will delete everything.")) {
        saveHabits([]);
        render();
      }
    });
